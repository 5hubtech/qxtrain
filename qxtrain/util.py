...
def setup_logging(log_filter_type: LogFilterType = LogFilterType.rank0_only) -> None:
    ...
    if (
        os.environ.get("qxtrain_NONINTERACTIVE", False)
...
def excepthook(exctype, value, traceback):
    ...
    elif issubclass(exctype, qxtrainCliError):
        rich.get_console().print(f"[yellow]{value}[/]", highlight=False)
    elif issubclass(exctype, qxtrainError):
...
def _get_s3_client(scheme: str):
    ...
    return session.client(
        ...
        use_ssl=not int(os.environ.get("qxtrain_NO_SSL", "0")),
    )
...
def _s3_upload(
    ...
    if not save_overwrite:
        ...
        if err is not None:
            raise qxtrainNetworkError(f"Failed to check object existence during {scheme} upload") from err

    try:
        _get_s3_client(scheme).upload_file(source, bucket_name, key)
    except boto_exceptions.ClientError as e:
        raise qxtrainNetworkError(f"Failed to upload to {scheme}") from e
...
def _s3_file_size(scheme: str, bucket_name: str, key: str, max_attempts: int = 3) -> int:
    ...
    raise qxtrainNetworkError(f"Failed to get {scheme} file size") from err
...
def _s3_get_bytes_range(
    ...
    raise qxtrainNetworkError(f"Failed to get bytes range from {scheme}") from err
...
def default_thread_count() -> int:
    return int(os.environ.get("qxtrain_NUM_THREADS") or min(32, (os.cpu_count() or 1) + 4))
...
def threaded_generator(g, maxsize: int = 16, thread_name: Optional[str] = None):
    ...
    for x in iter(q.get, sentinel):
        if isinstance(x, Exception):
            raise qxtrainThreadError(f"generator thread {thread_name} failed") from x
        else:
            yield x
