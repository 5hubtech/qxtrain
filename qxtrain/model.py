...
from .exceptions import qxtrainConfigurationError
...
__all__ = [
    ...
    "SwiGLU",
    "qxtrainBlock",
    "qxtrainSequentialBlock",
    "qxtrain",
    "qxtrainOutput",
    "qxtrainGenerateOutput",
]
...
class qxtrainBlock(nn.Module):
    ...
    @classmethod
    def build(cls, layer_id: int, config: ModelConfig, cache: BufferCache) -> qxtrainBlock:
        if config.block_type == BlockType.sequential:
            return qxtrainSequentialBlock(layer_id, config, cache)
        elif config.block_type == BlockType.llama:
            return qxtrainLlamaBlock(layer_id, config, cache)
        else:
            raise NotImplementedError(f"Unknown block type: '{config.block_type}'")


class qxtrainSequentialBlock(qxtrainBlock):
    """
...
class qxtrainLlamaBlock(qxtrainBlock):
    """
    This is a transformer block where the output is computed as ``MLP(LN(x + Attention(LN(x))))``
    (plus another skip connection). This block is similar to `qxtrainSequentialBlock`
...
class qxtrainOutput(NamedTuple):
...
class qxtrainGenerateOutput(NamedTuple):
...
class qxtrainBlockGroup(nn.ModuleList):
...
class qxtrain(nn.Module):
    def __init__(self, config: ModelConfig, init_params: bool = True):
        ...
        if self.config.alibi and self.config.flash_attention:
            raise qxtrainConfigurationError("ALiBi is currently not supported with FlashAttention")

        if self.config.alibi and self.config.rope:
            raise qxtrainConfigurationError("ALiBi and RoPE are mutually exclusive")

        if self.config.embedding_size is not None and self.config.embedding_size != self.config.vocab_size:
            if self.config.embedding_size < self.config.vocab_size:
                raise qxtrainConfigurationError("embedding size should be at least as big as vocab size")
        ...
        if not (
            ...
            raise qxtrainConfigurationError("n layers must be divisible by block group size")
        ...
        blocks = [qxtrainBlock.build(i, config, self.__cache) for i in range(config.n_layers)]
        if self.config.block_group_size > 1:
            block_groups = [
                qxtrainBlockGroup(config, i, blocks[i : i + config.block_group_size])
    ...
    def forward(
        ...
        return qxtrainOutput(logits=logits, attn_key_values=attn_key_values, hidden_states=tuple(all_hidden_states) if output_hidden_states else None)  # type: ignore[arg-type]
    ...
    def get_fsdp_wrap_policy(self, wrap_strategy: Optional[FSDPWrapStrategy] = None):
        ...
        if wrap_strategy == FSDPWrapStrategy.by_block:
            ...
            def fsdp_wrap_fn(module, recurse: bool = True, nonwrapped_numel: int = 0):
                ...
                wrap = isinstance(module, qxtrainBlock)
                if recurse:
                    return True
                else:
                    return wrap
            ...
            def fsdp_wrap_fn(module, recurse: bool = True, nonwrapped_numel: int = 0):
                del nonwrapped_numel
                wrap = isinstance(module, (qxtrainBlock,)) or module in size_based_module_to_wrap
                if recurse:
                    return True
                else:
                    return wrap
            ...
            def fsdp_wrap_fn(module, recurse: bool = True, nonwrapped_numel: int = 0):
                del nonwrapped_numel
                wrap = isinstance(module, (qxtrainBlockGroup,)) or module in size_based_module_to_wrap
                if recurse:
                    return True
                else:
                    return wrap
            ...
            def fsdp_wrap_fn(module, recurse: bool = True, nonwrapped_numel: int = 0):
                del nonwrapped_numel
                wrap = isinstance(module, qxtrainBlock) and module.layer_id % c == 0
                if recurse:
                    return True
                else:
                    return wrap
    ...
    def from_checkpoint(
        ...
        """
        Load an qxtrain model from a checkpoint.
        """
