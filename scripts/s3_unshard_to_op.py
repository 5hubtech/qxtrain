...
Example usage for `qxtrain_core`-style checkpoint (circa April 2024):
python scripts/s3_unshard_to_hf.py \
    --sharded_bucket s3://ai2-llm/checkpoints/qxtrain-medium/mitchish7/step239000 \
    --unsharded_bucket s3://ai2-llm/checkpoints/qxtrain-medium/mitchish7/step239000-unsharded \
    --hf_bucket s3://ai2-llm/checkpoints/qxtrain-medium/mitchish7/step239000-huggingface \
    --type qxtrain_core \
    --tmp_dir /net/nfs.cirrascale/allennlp/davidw/tmp/unshard

NOTE: For this to work, you need to install the `qxtrain-core` repo as follows:
- Clone https://github.com/allenai/qxtrain-core
...
def make_parser():
    parser = argparse.ArgumentParser(
        description="Unshard S3 checkpoint and convert to HF format. Invoke this script from the root of the qxtrain repo."
    )
...
def s3_unshard_to_hf(args):
    ...
    hf_cmd = f"python hf_qxtrain/convert_qxtrain_to_hf.py --checkpoint-dir {unsharded_dir}"
