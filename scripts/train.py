...
from op_qxtrain.config import CheckpointType, TrainConfig
from op_qxtrain.data import build_train_dataloader
from op_qxtrain.eval import build_evaluators
from op_qxtrain.exceptions import qxtrainCliError, qxtrainConfigurationError
from op_qxtrain.model import qxtrain
from op_qxtrain.optim import BoltOnWarmupScheduler, build_optimizer, build_scheduler
from op_qxtrain.torch_util import (
    barrier,
    get_default_device,
    get_global_rank,
    get_local_rank,
...
from op_qxtrain.train import Trainer
from op_qxtrain.util import (
...
def main(cfg: TrainConfig) -> None:
    ...
    if cfg.run_name is None:
        raise qxtrainConfigurationError("--run_name is required")
    ...
    if get_global_rank() == 0:
        ...
        if not cfg.dry_run and (cfg.load_path is None or Path(cfg.load_path).parent != Path(cfg.save_folder)):
            ...
            if save_path.is_file() and not cfg.save_overwrite:
                raise qxtrainConfigurationError(f"{save_path} already exists, use --save_overwrite to overwrite")
            else:
    ...
    qxtrain_model = qxtrain(cfg.model)
    log.info(f"Total number of parameters: {qxtrain_model.num_params():,d}")
    log.info(f"Number of non-embedding parameters: {qxtrain_model.num_params(include_embedding=False):,d}")
    log.info(f"Peak GPU Memory (MB) before FSDP: {int(peak_gpu_memory() or 0)}")

    qxtrain_model.set_activation_checkpointing(cfg.activation_checkpointing)

    # Wrap the model in FSDP.
    log.info("Wrapping model with FSDP...")
    wrap_policy = qxtrain_model.get_fsdp_wrap_policy(cfg.fsdp.wrapping_strategy)
    ...
    if cfg.fsdp.sharding_strategy in (ShardingStrategy.HYBRID_SHARD, ShardingStrategy._HYBRID_SHARD_ZERO2):
        if version.parse(torch.__version__) < version.parse("2.2.0"):
            ...
            raise qxtrainConfigurationError(
                "qxtrain training does not correctly support hybrid sharding before torch 2.2.0"
            )
        ...
        if num_model_replicas <= 0:
            raise qxtrainConfigurationError("fsdp.hybrid_sharding_num_model_replicas must be a positive integer")

        if get_world_size() % num_model_replicas != 0:
            raise qxtrainConfigurationError("fsdp.hybrid_sharding_num_model_replicas must divide world size")
    ...
    if param_init_fn is not None:
        qxtrain_model.reset_parameters()
    ...
    if cfg.save_data_indices:
        ...
        if indices_file_path.exists() and not cfg.save_overwrite:
            raise qxtrainConfigurationError(f"{indices_file_path} already exists, use --save_overwrite to overwrite")
...
if __name__ == "__main__":
    ...
    try:
        ...
        raise qxtrainCliError(f"Usage: {sys.argv[0]} [CONFIG_PATH] [OPTIONS]")
